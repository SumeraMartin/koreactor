...
class IntervalBehaviourTest : BaseBehaviourTest() {
    ...
    @Test
    fun `with normal flow emits`() {
        val behaviour = IntervalBehaviour<TestState>(
                triggers = triggers(initialSubject),
                duration = 1,
                timeUnit = TimeUnit.SECONDS,
                onTick = dispatch({ Output(id = it.toString()) })
        ).createObservable()
    ...
    fun triggeredAndCancelled_shouldStopEmitting() {
        val behaviour = IntervalBehaviour<TestState>(
                triggers = triggers(initialSubject),
                cancelTriggers = triggers(cancelSubject),
                duration = 2,
                timeUnit = TimeUnit.SECONDS,
                onTick = dispatch({ Output(id = it.toString()) })
        ).createObservable()
